	...
	void on_imgui_update() override
	{
...

		{
			...
			ImGui::Text("draw calls: %d",
			            sol::Renderer2D::get_statistics().get_draw_calls());
			ImGui::Text("quad count: %d",
			            sol::Renderer2D::get_statistics().get_quad_count());
			ImGui::Text("index count: %d",
			            sol::Renderer2D::get_statistics().total_index_count());
			ImGui::Text("vertex count: %d",
			            sol::Renderer2D::get_statistics().total_vertex_count());

			ImGui::End();
			sol::Renderer2D::get_statistics().reset();
		}
